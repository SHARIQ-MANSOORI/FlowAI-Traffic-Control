...
export function RoadSegment({ road, position }: RoadSegmentProps) {
  ...
  // Render little car dots based on vehicle count
  const renderCars = () => {
    // Max visual cars to not overload UI
    ...
    return Array.from({ length: visualCount }).map((_, i) => (
      <div 
        ...
        className={cn(
          ...
          road.isEmergency && i === 0 ? "bg-destructive animate-pulse w-4 h-5" : "bg-blue-200/80"
        )}
      />
    ));
  };
  ...
  const getDirectionIcon = () => {
    switch(position) {
      case "north": return <ArrowDown className="text-white/30 w-8 h-8" />; // Flowing towards center
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        // Position the cars appropriately depending on direction to center
      ...
      <AnimatePresence>
        {road.isEmergency && (
          <motion.div 
            ...
            style={{
              boxShadow: 'inset 0 0 50px rgba(255,50,50,0.2)'
            }}
          >
      ...
      <div className="absolute opacity-50 z-0">
        {getDirectionIcon()}
      </div>
